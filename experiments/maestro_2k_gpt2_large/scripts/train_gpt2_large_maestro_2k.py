...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train a GPT-2 Large style piano model on the isolated Maestro 2k token set."
    )
...
def main():
    ...
    if not META_PATH.exists():
        raise FileNotFoundError(f"tokenization metadata not found: {META_PATH}")
    ...
    train_ds = MidiWindowDataset(TOKENS / "train", args.block_size, args.stride)
    val_ds = MidiWindowDataset(TOKENS / "validation", args.block_size, args.stride)

    if len(train_ds) == 0 or len(val_ds) == 0:
        raise RuntimeError("train/validation token windows are empty; check split prep and tokenization first")
    ...
    config = build_model_config(vocab_size=vocab_size, block_size=args.block_size)
    ...
    if resume_stats is not None:
        print(f"resume_from: {args.resume_from.expanduser().resolve()}", flush=True)
