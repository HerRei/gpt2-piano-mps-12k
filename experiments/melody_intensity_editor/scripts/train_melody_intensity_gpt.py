...
def parse_args():
    ...
    parser.add_argument("--log-every-opt-steps", type=int, default=10)
...
def update_latest_checkpoint(checkpoints_dir: Path, ckpt_dir: Path):
    ...
    if latest_link.is_symlink() or latest_link.exists():
        if latest_link.is_dir() and not latest_link.is_symlink():
            ...
            latest_link.unlink()
    try:
        os.symlink(ckpt_dir.name, latest_link, target_is_directory=True)
        if latest_txt.exists():
            latest_txt.unlink()
    except OSError:
        latest_txt.write_text(str(ckpt_dir))
...
def main():
    global ARTIFACTS, EXAMPLES, META_PATH, CHECKPOINTS, LOGS, STATUS_PATH
    args = parse_args()
    ...
    if args.resume_from is None:
        ...
        if trainer_state_path.exists():
            resume_state = load_torch_payload(trainer_state_path)
            previous_total_elapsed = float(resume_state.get("total_elapsed_sec", previous_total_elapsed))
    ...
    if args.epochs < start_epoch:
        raise ValueError(
            f"epochs={args.epochs} is too small for resume checkpoint epoch={start_epoch - 1}; "
            "set --epochs to the total desired epoch count"
        )
    ...
    fallback_global_opt_step = (start_epoch - 1) * optimizer_steps_per_epoch

    if resume_state is None:
        global_opt_step = fallback_global_opt_step
        ...
        global_opt_step = int(resume_state.get("global_optimizer_step", fallback_global_opt_step))
        scheduler = get_cosine_schedule_with_warmup(
            optimizer,
            num_warmup_steps=args.warmup_steps,
            num_training_steps=total_optimizer_steps,
    ...
    try:
        for epoch in range(start_epoch, args.epochs + 1):
            ...
            update_latest_checkpoint(CHECKPOINTS, ckpt_dir)
