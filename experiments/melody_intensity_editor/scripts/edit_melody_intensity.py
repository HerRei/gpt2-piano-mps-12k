...
def normalize_score_start(score: Score) -> Score:
    merged = merge_score(score)
    notes = list(merged.tracks[0].notes)
    pedals = list(merged.tracks[0].pedals)
    anchor_candidates = [int(note.time) for note in notes]
    anchor_candidates.extend(int(pedal.time) for pedal in pedals)
    ...
    shifted_pedals = []
    for pedal in pedals:
        start = max(0, int(pedal.time) - anchor)
        end = max(start + 1, int(pedal.time) + max(1, int(pedal.duration)) - anchor)
        shifted_pedals.append(Pedal(start, end - start))

    tempos = []
    active_tempo = None
    for tempo in merged.tempos:
        tempo_time = int(tempo.time)
        if tempo_time <= anchor:
            active_tempo = Tempo(0, qpm=float(tempo.qpm))
        else:
            tempos.append(Tempo(tempo_time - anchor, qpm=float(tempo.qpm)))
    if active_tempo is None:
        active_tempo = Tempo(0, qpm=float(merged.tempos[0].qpm)) if merged.tempos else Tempo(0, qpm=120.0)
    tempos.insert(0, active_tempo)

    time_signatures = []
    active_signature = None
    for signature in merged.time_signatures:
        signature_time = int(signature.time)
        if signature_time <= anchor:
            active_signature = TimeSignature(0, int(signature.numerator), int(signature.denominator))
        else:
            time_signatures.append(
                TimeSignature(signature_time - anchor, int(signature.numerator), int(signature.denominator))
            )
    if active_signature is None:
        if merged.time_signatures:
            first = merged.time_signatures[0]
            active_signature = TimeSignature(0, int(first.numerator), int(first.denominator))
        else:
            active_signature = TimeSignature(0, 4, 4)
    time_signatures.insert(0, active_signature)

    return create_score_from_components(
        int(merged.ticks_per_quarter),
        shifted_notes,
        shifted_pedals,
        tempos,
        time_signatures,
...
def convert_score_tpq(score: Score, target_tpq: int) -> Score:
    merged = merge_score(score)
    source_tpq = int(merged.ticks_per_quarter)
    target_tpq = int(target_tpq)
    if source_tpq <= 0 or target_tpq <= 0 or source_tpq == target_tpq:
        return merged

    scale = target_tpq / source_tpq

    notes = [
        Note(
            int(round(int(note.time) * scale)),
            max(1, int(round(max(1, int(note.duration)) * scale))),
            int(note.pitch),
            int(note.velocity),
        )
        for note in merged.tracks[0].notes
    ]

    pedals = []
    for pedal in merged.tracks[0].pedals:
        start = int(round(int(pedal.time) * scale))
        duration = max(1, int(round(max(1, int(pedal.duration)) * scale)))
        pedals.append(Pedal(start, duration))

    tempos = [Tempo(int(round(int(tempo.time) * scale)), qpm=float(tempo.qpm)) for tempo in merged.tempos]
    time_signatures = [
        TimeSignature(int(round(int(signature.time) * scale)), int(signature.numerator), int(signature.denominator))
        for signature in merged.time_signatures
    ]

    return create_score_from_components(target_tpq, notes, pedals, tempos, time_signatures)


def extract_melody(score: Score) -> Score:
    merged = normalize_score_start(score)
...
def main():
    ...
    target_score = normalize_score_start(tokenizer.decode([target_ids]))
    target_score = convert_score_tpq(target_score, int(prompt_score.ticks_per_quarter))
    target_score.dump_midi(str(output_path.resolve()))
